...
async def _run_host_workers(
    hosts: List[str], worker: Callable[[str], Dict[str, Any]], max_parallel: int
) -> List[Dict[str, Any]]:
    """Execute *worker* for each host with an upper bound on concurrency."""

    limit = max(1, min(max_parallel or 1, len(hosts) or 1))
    semaphore = asyncio.Semaphore(limit)

    async def run(host: str) -> Dict[str, Any]:
        async with semaphore:
            return await asyncio.to_thread(worker, host)

    tasks = [asyncio.create_task(run(host)) for host in hosts]
    ...
    for host, result in zip(hosts, results):
        if isinstance(result, Exception):
            ...
            normalized.append({"host": host, "error": message})
        else:
            normalized.append(result)
    return normalized


def _resolve_max_parallel(value: Any, total_hosts: int, default: int = 4) -> int:
    """Return a sanitized parallelism level for host collection."""

    total = max(1, total_hosts)
    default_limit = max(1, min(default, total))
    ...
    try:
        ...
        raise ValueError("max_parallel must be a positive integer") from exc
    ...
    if content_type.startswith("application/json"):
        ...
        try:
            max_parallel = _resolve_max_parallel(
                data.get("max_parallel"), len(hosts), default=8
            )
        ...
        def worker(host: str) -> Dict[str, Any]:
            try:
        ...
        host_outcomes = await _run_host_workers(hosts, worker, max_parallel)

        for outcome in host_outcomes:
            host = outcome.get("host", "")
            if "error" in outcome:
                errors.append({"host": host, "error": str(outcome["error"])})
                continue
